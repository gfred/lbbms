 ...
 */
@Stateless
@Path(CurrentLocationRessource.URI_TEMPALTE)
public class CurrentLocationRessource {
    public static final String URI_TEMPALTE = SingleCustomerResource.URI_TEMPALTE + "/location";
    ...
    public LocationRepresentation getCurrentLocationByCustomerId(@PathParam(ResourceValues.CUSTOMER_ID) Long id){
    ...
    @Produces({MediaType.APPLICATION_JSON,MediaType.APPLICATION_XML})
    public LocationRepresentation updateCurrentLocationByCustomerId(@PathParam(ResourceValues.CUSTOMER_ID) Long id, LocationRepresentation rep){
        if(id==null || customerAdmin.getCustomerById(id)==null){
            throw new WebApplicationException(Status.NOT_FOUND);
        }
